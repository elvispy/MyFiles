...
\begin{sol}
	...
	\centering{$\sum_{\substack{1\leq i < j < k \leq 4}} \frac{a_{i}^3+a_{j}^{3}+a_{k}^{3}}{a_{i}+a_{j}+a_{k}} \geq \Sigma_{1\leq i < j < k \leq 4} \frac{a_{i}^2+a_{j}^{2}+a_{k}^{2}}{3} = a_{1}^{2}+a_{2}^{2}+a_{3}^{2}+a_{4}^{2}\hspace{0.1cm} \square$}
\end{sol}
