...
function solveMotion2_0(rS, Tm, R_f, mu, mS, g, options)
    %mainv_1.9
    %Tries to solve the kinematic match between a rigid spherical object
    %and an axissymmetric membrane with air pressure, Newton's Method and 
    % Euler Implicit finite differences
    ...
    AA = returnMatrices2_0(AA, Ntot, dr, Mr);
    ...
    while (t <= finalTime)
        ...
        if isempty(AA{cPoints + 3}) == true
            AA = returnMatrices1_9(AA, Ntot, dr, Mr);
        end
        ...
        [Eta_k_prob(:,3), u_k_prob(:, 3), z_k_prob(3), ...
            ...
            Eta_k, u_k, z_k, v_k, P_k, dt, dr, Fr, Mr, Ntot, AA, D);
...
%                 delete(tt);
    ...
    if options.exportData == true 
        ...
        save(fullfile(pwd, sprintf('/simulations/simul%g_%g_%s.mat', Tm, rS ,vars{1})), ...
            ...
            'recordedv_k', 'cTime', 'labcTime', 'maxDef', 'Tm', 'mu', 'rS', 'rt', 'v_k', 'N', 'mS', ...
