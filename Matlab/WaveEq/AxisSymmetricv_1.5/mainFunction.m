...
function mainFunction(rS, Tm, R_f, mu, mS, g, options)
    %mainv_1.4
    ...
    %Numerical constants
    N = (ceil(10 * rS)+15); %Number of dx intervals in the spatial (radial) coordinate per unit length
    ...
    while (ii <= mii)
        ...
        if recalculate == true
            ...
            elseif (cVar == true && cPoints > 0)
                ctime = ctime + dt;
            elseif (cVar == true && cPoints == 0 ...
                    && options.saveAfterContactEnded == false)
