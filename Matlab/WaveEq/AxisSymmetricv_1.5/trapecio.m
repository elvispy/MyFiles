function [Eta_k_prob, u_k_prob, z_k_prob, v_k_prob, P_k_prob, errortan] = ...
    ...
    if newCPoints < 0 || newCPoints > mCPoints
        ...
        %% Building A
        %We will construct A succesively, dividing it into a number of parts
        % corresponding to the number of equations and variables 
        ...
        A_1 = [eye(Ntot); A_prime; zeros(2, Ntot)]; %We are using implicit Euler       
        %A_1((Ntot + 1):(Ntot + newCPoints), :) = zeros(newCPoints, Ntot); %We are using the curvature operator
        C1 = A_1(:, 1:newCPoints); %Eta_k is written in terms of z_k
        ...
        %Now we replace by zeros some rows, as they appear in curvature
        %form
        %B_1((Ntot + 1):(Ntot + oldCPoints), :) = zeros(oldCPoints, Ntot);
        
        %Second column block (u_k)
        B_2 = [dt/2 * zeros(Ntot); eye(Ntot); zeros(2, Ntot)];
        ...
        B = [B_1 B_2 B_3 B_4];
        B = B((newCPoints + 1):end, :);
        
        %% Building R and R_prime
        
        %change this if your dont want to take into account gravity
        R = [zeros(Ntot-newCPoints, 1); (-Fr * dt) * ones(Ntot, 1); 0 ; -Fr * dt]; %with gravity
        ...
        u_k_prob = [u_k_prob ;x((Ntot - newCPoints + 1):(2*Ntot - 2*newCPoints))];
        P_k_prob = x((2 * Ntot - 2*newCPoints + 1):(2*Ntot - newCPoints));
        
        
        assert(size(Eta_k_prob, 1) == Ntot);
        assert(size(u_k_prob, 1) == Ntot);
        assert(size(P_k_prob, 1) == newCPoints);
        assert(size(x, 1) == 2*Ntot - newCPoints + 2);
