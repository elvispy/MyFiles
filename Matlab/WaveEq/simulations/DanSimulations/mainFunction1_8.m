...
function ctime = mainFunction1_8(rS, Tm, R_f, mu, mS, g, options)
    ...
    if exist(FileName, 'file') == 0 %Check if .csv file exists
        writematrix(["ID", "vi", "surfaceTension", "radius", ...
            "cTime", "maxDeflection", "coefOfRestitution", "Density"], FileName, ...
    ...
    while (t <= finalTime)
        ...
        if recalculate == true % If calculations didnt go well
            ...
            elseif (contactFlag == true && recordedz_k(currentIndex - 1) >= Lunit * (ZERO_HEIGHT + 1) ...
                && ch == false) % record last contact time
...
%                         delete(tt);
                ...
                if options.saveAfterContactEnded == false && cPoints == 0
...
%                 delete(tt);
    ...
    if options.exportData == true 
        ...
        save(fullfile(pwd, sprintf('/simulations/simul%g_%g_%s.mat', Tm, rS ,vars{1})), ...
            ...
            'coefOfRestitution');
        writecell([vars, ctime, maxDef, coefOfRestitution, 3*mS/(4*pi*rS^3)], FileName, ...
           'WriteMode', 'append');
    end
