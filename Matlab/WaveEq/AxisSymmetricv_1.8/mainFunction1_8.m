...
function mainFunction1_8(rS, Tm, R_f, mu, mS, g, options)
    ...
    if options.saveAfterContactEnded == false
        options.SimulTime = 20; % Change this later on
    end
    ...
    %Numerical constants
    N = 25; %ceil(2500/R_f);% ceil(10*rS.^(8/5)) + 15;%   %Number of dx intervals in the spatial (radial) coordinate per unit length
    ...
    maxDeflectionRecorded = false; % To check if max deflextion was recorded
    ...
    AA = returnMatrices1_8(mCPoints, Ntot, dr, Mr);
    ...
    while (t <= finalTime)
        ...
        if recalculate == true % If calculations didnt go well
            ...
            if (contactFlag == false && cPoints > 0) % if contact began, start counting
                ...
                vars(2) = round(recordedv_k(currentIndex - 2, 1), 5); % Store initial impact velocity
            ...
            elseif (contactFlag == true && cPoints > 0) % count for contact time
                ctime = ctime + dt;
            elseif (contactFlag == true && cPoints == 0) % record last contact time
                if maxDeflectionRecorded == false
                    ...
                    if options.plotter == true
                        aaa = 1;
%                         tt = text(-2.4, 2.7, '$c)$', 'Interpreter', 'latex', 'FontSize', 16);
%                         print(myFigure, '-depsc', '-r300', 'Graficos/TakeOff.eps'); 
%                         delete(tt);
                    end
...
%                 delete(tt);
    ...
    if options.exportData == true 
        ...
        %Cut data
        recordedz_k = recordedz_k(1:(currentIndex-1));
        recordedEta = recordedEta(1:(currentIndex-1), :);
        recordedu_k = recordedu_k(1:(currentIndex-1), :);
        recordedPk = recordedPk(1:(currentIndex-1), :);
        recordedv_k = recordedv_k(1:(currentIndex-1), :);
        recordedTimes = recordedTimes(1:(currentIndex - 1), :);
    ...
    fprintf('Coefficient of Restitution squared: %0.3f \n', coefOfRestitution);
    fprintf('dt: %0.5g (ms)\n', dt*Tunit);
    fprintf('dr: %0.5g (mm)\n', dr * Lunit);
    disp('-----------------------------');
